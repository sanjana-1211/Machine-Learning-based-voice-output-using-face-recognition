...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'myteam@gmail.com' ")
...
def TrackImages():
    check_haarcascadefile()
    assure_path_exists("recognization/")
    assure_path_exists("StudentDetails/")
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(aa)
                bb = bb[2:-2]
                recognization = [str(ID), '', bb, '', str(date), '', str(timeStamp)]
                t1 = gtts.gTTS("hai good morning"+ bb +"welcome")
                t1.save("welcome.mp3")
        ...
        cv2.imshow('Taking recognization', im)

        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("recognization\recognization" + date + ".csv")
    if exists:
        with open("recognization\recognization" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            writer.writerow(recognization)
        csvFile1.close()
    else:
        with open("recognization\recognization" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            ...
            writer.writerow(recognization)
        csvFile1.close()
    with open("recognization\recognization" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
message3 = tk.Label(window, text="Face Recognition System using python" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
lbl3 = tk.Label(frame1, text="recognization",width=20  ,fg="black"  ,bg="#00aeff"  ,height=1 ,font=('times', 17, ' bold '))
...
trackImg = tk.Button(frame1, text="check Recoginization", command=TrackImages  ,fg="black"  ,bg="yellow"  ,width=35  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
